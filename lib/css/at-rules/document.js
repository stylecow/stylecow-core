...
stylecow.Document = class extends require('../../classes/node-collection-vendor') {
    ...
    static create (reader) {
        if (reader.currToken === AT && reader.nextToken === NAME && reader.nextStr.endsWith('document')) {
            reader.move();

            let element = (new stylecow.Document(reader.data()))
