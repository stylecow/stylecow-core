...
stylecow.Declaration = class extends require('../classes/node-collection-vendor') {
	...
	static create (reader) {
		...
		if (reader.currToken === NAME) {
			element = (new stylecow.Declaration(reader)).setNameWithVendor(reader.getStringAndMove());
	...
	static createMsFilter (reader) {
		...
		if (reader.currToken === NAME) {
			...
			var element = (new stylecow.Declaration(reader))
				.setName('filter')
				.setVendor('ms');
			...
			if (reader.currToken === NAME && reader.currStr === 'none') {
				...
				var filter = (new stylecow.String(reader)).setName('');
