...
export default class Declaration extends NodeCollectionVendor {
  static create(reader, parent) {
    ...
    if (reader.currToken === NAME) {
      element = (new Declaration(reader.data())).setNameWithVendor(
    ...
    if (
      element.name === "filter" &&
      (element.vendor === "ms" ||
        reader.searchNext([CLOSE_CURLY_BRACKET, SEMICOLON, EQUALS, EOF]) ===
          EQUALS)
    ) {
      element.vendor = "ms";
      ...
      if (reader.currToken === NAME && reader.currStr === "none") {
        ...
        do {
          ...
          reader.move() && reader.currToken !== SEMICOLON &&
  ...
  toString() {
    let string = this.getNameWithVendor() + ": " +
