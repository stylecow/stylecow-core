...
stylecow.Function = class extends require('../classes/node-collection-vendor') {
	...
	constructor(reader) {
		super(reader, 'Function');
	}

	static create (reader) {
		if (reader.currToken === NAME && reader.nextToken === OPEN_PARENTHESIS) {
			var element = (new stylecow.Function(reader)).setNameWithVendor(reader.getStringAndMove());
	...
	static createUrl (reader, fromString, names) {
		...
		if (reader.currToken === NAME && reader.nextToken === OPEN_PARENTHESIS && (names.indexOf(reader.currStr) !== -1)) {
			var element = (new stylecow.Function(reader)).setName(reader.getStringAndMove());
			...
			// url(address)
				...
				var url = (new stylecow.String(reader)).setName('');
		...
		if (fromString && reader.currToken === STRING) {
			var element = (new stylecow.Function(reader)).setName('url');
