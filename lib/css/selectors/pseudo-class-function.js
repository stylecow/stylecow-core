...
stylecow.PseudoClassFunction = class extends require('../../classes/node-collection-vendor') {
	...
	constructor(reader) {
		super(reader, 'PseudoClass', 'PseudoClassFunction');
	}
	...
	static create (reader) {
		if (reader.currToken === COLON && reader.nextToken === NAME && reader.nextNextToken === OPEN_PARENTHESIS) {
			...
			var element = (new stylecow.PseudoClassFunction(reader)).setNameWithVendor(reader.getStringAndMove());
	...
	static createPosition (reader) {
		if (reader.currToken === COLON && reader.nextToken === NAME && reader.nextStr.match(/^(-(\w+)-)?(nth-.+)$/)) {
			var element = (new stylecow.PseudoClassFunction(reader)).setNameWithVendor(reader.nextStr);
			...
			var value = new stylecow.Value(reader);
			element.push(value);
			...
			// an+b
				...
				if (reader.currToken === NUMBER && reader.nextToken === NAME && reader.nextStr === 'n') {
					...
					var unit = (new stylecow.Unit(reader)).setName(reader.getStringAndMove());

					unit.push((new stylecow.Number(reader)).setName(1));
				...
				if (reader.currToken === NUMBER) {
					value.push((new stylecow.Operator(reader)).setName('+'));
					value.push(stylecow.Number.create(reader) || reader.error());
				} else {
