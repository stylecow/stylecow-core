...
stylecow.Value = class extends require('../classes/node-collection') {
	...
	static create (reader) {
		...
		do {
			if (reader.currToken[0] === CLOSE_PARENTHESIS || reader.currToken[0] === EXCLAMATION || reader.currToken[0] === SEMICOLON || reader.currToken[0] === CLOSE_CURLY_BRACKET || reader.currToken[0] === OPEN_CURLY_BRACKET || reader.currToken[0] === COMMA || reader.currToken[0] === EOF) {
				break;
			}

			if (reader.currToken[0] === SOLIDUS || reader.currToken[0] === PLUS) {
		...
		} while ((reader.get(-2)[0] === WHITESPACE) || reader.currToken[0] === SOLIDUS);
