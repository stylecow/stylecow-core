...
function prepareTasks(allTasks, support) {
  ...
  for (i = allTasks.length - 1; i >= 0; i--) {
    ...
    if (typeof task.filter === "string") {
      ...
      types = (task.filter.type instanceof Array
        ? task.filter.type
        : (task.filter.type
          ? [task.filter.type]
          : ["_"]));
      names = (task.filter.name instanceof Array
        ? task.filter.name
        : (task.filter.name ? [task.filter.name] : ["_"]));
    }
...
function needFix(stylecowSupport, task, method) {
  ...
  for (let browser in taskSupport) {
    ...
    if (
      method === "forBrowsersLowerThan" &&
      (taskSupport[browser] === false ||
        stylecowSupport[browser] < taskSupport[browser])
    ) {
      return true;
    }
