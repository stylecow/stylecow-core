...
stylecow.Tasks = class {
    ...
    run (root) {
        if (!this.preparedTasks) {
            this.preparedTasks = prepareTasks(this.tasks, this.support);
        }

        executeRootTasks(root, this.preparedTasks);
...
function prepareTasks (allTasks, support) {
    ...
    for (i = allTasks.length - 1; i >= 0; i--) {
        ...
        if (typeof task.filter === 'string') {
            ...
            types = (task.filter.type instanceof Array ? task.filter.type : [task.filter.type]) || ['_'];
            names = (task.filter.name instanceof Array ? task.filter.name : [task.filter.name]) || ['_'];
        }
...
function executeRootTasks (element, tasks) {
    ...
    if (tasks.before.hasOwnProperty('Root')) {
        if (tasks.before.Root.hasOwnProperty('_')) {
            execRootTasks(element, tasks.before.Root._);
        }
    }
    ...
    if (tasks.after.hasOwnProperty('Root')) {
        if (tasks.after.Root.hasOwnProperty('_')) {
            execRootTasks(element, tasks.after.Root._);
        }
    }
}
...
function executeChildTasks (element, tasks) {
    ...
    if (tasks.before[element.type] !== undefined) {
    ...
    if (tasks.after.hasOwnProperty(element.type)) {
        let t = tasks.after[element.type];
