...
stylecow.Tokens = class {
    ...
    SPECIAL_CHAR () {
        if (specialChars.has(this.currChar)) {
            return [specialChars.get(this.currChar), this.line, this.col, this.currChar];
        }
    }
    ...
    NUMBER () {
        if (
            isDigit(this.currChar)
             || ((this.currChar === '-') && ((this.nextChar === '.' && isDigit(this.nextNextChar)) || isDigit(this.nextChar)))
             || ((this.currChar === '.') && isDigit(this.nextChar))
        ) {
            var token = [NUMBER, this.line, this.col, this.currChar];
    ...
    STRING () {
        if (this.currChar === '"' || this.currChar === "'") {
            ...
            var token = [STRING, this.line, this.col, ''];
    ...
    NAME () {
        if (isEscaped(this.currChar, this.nextChar) || (isName(this.currChar) && (this.currChar !== '-' || (isName(this.nextChar) && !isDigit(this.nextChar))))) {
            var token = [NAME, this.line, this.col, this.currChar];
    ...
    HASH () {
        if (this.currChar === '#') {
            var token = [HASH, this.line, this.col, ''];
